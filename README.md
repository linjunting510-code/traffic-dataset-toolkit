...
<视频名>_frame_000001.jpg 连续帧图片
  ...
  │  (可选,推荐)tds subsample ──► 每 N 帧取 1 帧抽稀,先抽稀再标注,减少人工量
  │  自动标注(二选一或结合):
...
```bash
# 每 5 帧取 1 帧,复制到新事件目录(保留原文件名)
tds subsample "D:\dataset_work\pilot\国庆事件" -o "D:\dataset_work\dataset\国庆事件_5to1"

# 换起始位移;覆盖已有目录需显式 --force
tds subsample "D:\dataset_work\pilot\国庆事件" -o "D:\dataset_work\dataset\x" --every 5 --offset 2 --force
```

固定监控机位下相邻帧近似重复,正式数据集按"每 5 帧标 1 帧"抽稀(例子数据集 001537 即如此)。**建议在标注之前抽稀**,直接减少人工量。选中的图片复制到 `<新目录>/images/` 并保留原文件名(帧号可追溯回源视频);源有 `labels/` 时同名 txt 和 classes.txt 一并复制(autolabel_report.txt 等元文件不复制,抽稀后旧报告会过时);输出目录非空时拒绝写入,除非 `--force`。
...
- **环境**:需要在打标环境 `pip install ultralytics`(torch 随之安装,CPU 可跑);`--model` 默认 yolo11s.pt(首次自动下载;实测比 yolo11n 近处大车漏检、Bus/Truck 误分更少),`--conf` 默认 0.2(实测比 0.25 漏检更少),`--device` 默认 cpu
...
├── src/traffic_dataset/
│   ├── cli.py             # argparse 入口:extract/rename/subsample/autolabel/convert/validate
│   ├── classes.py         # 类别加载与解析(带 YAML 极简兜底)
│   ├── extract.py         # ffmpeg 视频抽帧
│   ├── rename.py          # 图片重命名
│   ├── subsample.py       # 帧抽稀(每 N 帧取 1 帧)
